...
let validPassports = passports.filter { passport in
    fields.filter { (field) -> Bool in
        !passport.contains(field)
    }.isEmpty
}

print("validPassports: \(validPassports.count)")

struct Height {
    let height: Int // (Height) - a number followed by either cm or in:
    ...
    init?(_ value: String) {
        if value.contains("cm") {
            let heightString = value.replacingOccurrences(of: "cm", with: "")
            guard let height = Int(heightString), height >= 150, height <= 193 else {
                return nil
            }
            self = .init(height: height, unit: "cm")
        } else if value.contains("in") {
            let heightString = value.replacingOccurrences(of: "in", with: "")
            guard let height = Int(heightString), height >= 59, height <= 76 else { return nil }
            self = .init(height: height, unit: "in")
        } else {
            return nil
        }
    }

    private init(height: Int, unit: String) {
        self.height = height
        self.unit = unit
    }
}
...
struct Passport {
    ...
    let ecl: EyeColor // (Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
    let pid: String // (Passport ID) - a nine-digit number, including leading zeroes.
    let cid: String? // (Country ID) - ignored, missing or not.

    init?(_ values: [String]) {
        var dict = [String: String]()
        values.map { item in
            let key = item.prefix(3)
            let value = item.suffix(from: item.index(item.startIndex, offsetBy: 4))
            dict[String(key)] = String(value)
        }

        guard let byrString = dict["byr"], let byr = Int(byrString), byr >= 1920, byr <= 2002 else { return nil }
        guard let iyrString = dict["iyr"], let iyr = Int(iyrString), iyr >= 2010, iyr <= 2020 else { return nil }
        guard let eyrString = dict["eyr"], let eyr = Int(eyrString), eyr >= 2020, eyr <= 2030 else { return nil }
        guard let hgtString = dict["hgt"], let hgt = Height(hgtString) else { return nil }
        guard let hclString = dict["hcl"], let regExp = try? NSRegularExpression(pattern: "^[#]{1}[0-9a-z]{6}$"),
              let firstMatch = regExp.firstMatch(in: hclString, options: [], range: NSRange(hclString.startIndex..., in: hclString)) else {
            return nil
        }
        guard let eclString = dict["ecl"], let ecl: EyeColor = EyeColor(rawValue: eclString) else { return nil }
        guard let pidString = dict["pid"], pidString.count == 9, let pid = Int(pidString) else { return nil }
...
let validatedPassports = validPassports
    .map { $0.replacingOccurrences(of: "\n", with: " ").components(separatedBy: " ") }
    .compactMap { Passport($0) }

print("validated: \(validatedPassports.count)")
