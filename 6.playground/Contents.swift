...
/// Part One
let count = groups.map { $0.replacingOccurrences(of: "\n", with: "") }
    .map { Set(Array($0)) }
    .map { $0.count }
    .reduce(0, +)

print("anyoneAnswered: \(count)") // 6249

/// Part Two
let everyoneAnsweredCount = groups.map { $0.split(separator: "\n") }
    .map { (group) -> Int in
        let availableAnswers = Set(group.flatMap { Array($0) })
        return availableAnswers.map { (answer) -> Int in
            group.allSatisfy { $0.contains(answer) } ? 1 : 0
        }.reduce(0, +)
    }.reduce(0, +)

print("everyone answered: \(everyoneAnsweredCount)") // 3103
