...
/// Part One
let partOne = inputs.map { input -> [String] in
    var row = input.split(separator: " ").map { String($0) }
    row[1] = row[1].replacingOccurrences(of: ":", with: "")
    return row
}.filter {
    let times: [Int] = $0[0].split(separator: "-").map { Int($0)! }
    let range = times[0]...times[1]
    let occurences = $0[2].components(separatedBy: $0[1]).count - 1
    return range.contains(occurences)
}.count

print("\(partOne) passwords are valid") //542

/// Part Two
let partTwo = inputs.map { input -> [String] in
    var row = input.split(separator: " ").map { String($0) }
    row[1] = row[1].replacingOccurrences(of: ":", with: "")
    return row
}.filter {
    let positions: [Int] = $0[0].split(separator: "-").map { Int($0)! - 1 }
    let firstPosition = positions[0]
    let secondPosition = positions[1]
    let password = $0[2]
    let first: String = String(password[password.index(password.startIndex, offsetBy: firstPosition)])
    let second: String = String(password[password.index(password.startIndex, offsetBy: secondPosition)])
...
print("\(partTwo) passwords are valid") // 360
