...
extension InnerBag: Equatable {}
extension InnerBag: Hashable {}
...
struct Rule {
    ...
    init(_ input: String) {
        let outerColor = input.components(separatedBy: " bags contain ")
        self.color = outerColor[0]

        if "no other bags." == outerColor[1] {
            self.innerBags = []
        } else {
            self.innerBags = outerColor[1].replacingOccurrences(of: ".", with: "")
                .components(separatedBy: ", ")
                .map { $0.replacingOccurrences(of: " bags", with: "").replacingOccurrences(of: " bag", with: "") }
                .map {
                    InnerBag(
                        color: String($0.suffix(from: $0.firstIndex(of: " ")!)).trimmingCharacters(in: [" "]),
                        count: Int($0.prefix(upTo: $0.firstIndex(of: " ")!))!)
                }
        }
    }
}
...
func containingColors(_ colors: String) -> [Rule] {
    var result = rules.filter { $0.innerBags.contains { (inner) -> Bool in
        inner.color == colors
    }}
    result.append(contentsOf: result.map { $0.color }.flatMap(containingColors))
    return result
}

let result = Set(containingColors("shiny gold"))
print(result.count) // 128

/// Part Two
func findRule(_ color: String) -> Rule {
    rules.filter { color == $0.color }.first!
}

func countInnerBags(_ rule: Rule) -> Int {
    rule.innerBags
        .map { $0.count * (1 + countInnerBags(findRule($0.color))) }
        .reduce(0, +)
}

var result2 = countInnerBags(findRule("shiny gold"))
print(result2) // 20189
